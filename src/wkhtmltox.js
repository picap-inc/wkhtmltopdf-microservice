...
function wkhtmltox(opts) {
    ...
    function launchWorker() {
        ...
        return spawn("/bin/sh", [ "-c", [ self.wkhtmltopdf, "--read-args-from-stdin" ].join(" ") ]);
    }

    // Given a worker process, stream the arguments and input over stdin and return stdout.
    function performWork(worker, inputStream, footerStream, options) {
        ...
        worker.stdin.write(" --footer-html ");
        worker.stdin.write(" - -\n");
        footerStream.pipe(worker.stdin);
    ...
    // HTML to PDF
    self.pdf = function pdf(inputStream, footerStream, options) {
        ...
        return performWork(worker, inputStream, footerStream, options);
    };
